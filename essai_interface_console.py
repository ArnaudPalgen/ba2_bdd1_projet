...
def add():
    ...
    depStr=printDep(dep)
    jr=input("Votre dependance a bien ete ajoutee "+ depStr)
    main_menu()
...
def analyse():
    ...
    if len(tableau)!= 0:
        ...
        for line in tableau:
            print(".  Table: "+line[0]+"relation: "+line[1]+" --> "+line[2])
    try:
            ...
            if opti ==1:
                ...
                for i in tableau:
                    print(isLogicConsequence(line[i-1][0],line[i-1][1],line[i-1][2])) # a verifier
                    main_menu()
